...
/*   Updated: 2019/07/07 19:13:20 by ciglesia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		if (fd < 0 || read(fd, guacal, 0) || (!ft_validdoc(fd)))
			return (closedoc("Invalid document!", fd));
		if (!(fd = open(argv[1], O_RDONLY)))
			return (-1);
		if (!(tetros = ft_identify(fd)))
			return (closedoc("Invalid Tetriminoes!", fd));
		if ((!ft_solve(tetros)))
			return (closedoc("Problems in solving!", fd));
	}
	else
